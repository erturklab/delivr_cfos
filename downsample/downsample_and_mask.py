...
def downsample_mask(settings, brain):
    ...
    if [dim < 250 for dim in (z,y,x)]:
        ...
        if z < 250:
            ds_padded = np.pad(ds_padded,pad_width=((0,256-z),(0,0),(0,0)),constant_values=0)
        if y < 250:
            ds_padded = np.pad(ds_padded,pad_width=((0,0),(0,256-y),(0,0)),constant_values=0)
        if x < 250:
            ds_padded = np.pad(ds_padded,pad_width=((0,0),(0,0),(0,256-x)),constant_values=0)
        downsampled_name = 'stack_resampled_padded_8bit.tif'
        io.imsave(os.path.join(results_folder,downsampled_name),ds_padded,compression='lzw')    

    else:
        downsampled_name = downsampled_name + '_8bit.tif'
    ...
    print("downsampled_name = " ,downsampled_name)
    save_vaa3d(teraconverter_path, os.path.join(results_folder,downsampled_name), downsampled_masked_vaa3d)
    ...
    if [dim < 250 for dim in (z,y,x)]:
        ...
        if x < 250:
            ds_padded = np.pad(ds_padded,pad_width=((0,0),(0,0),(0,256-x)),constant_values=0)
        downsampled_name = 'downsampled_masked_stack_padded_8bit.tif'        
        io.imsave(os.path.join(downsampled_masked_path,downsampled_name),ds_padded,compression='lzw')    
        
    else:
        downsampled_name = 'downsampled_masked_stack_8bit.tif'

    save_vaa3d(teraconverter_path, os.path.join(downsampled_masked_path,downsampled_name), downsampled_masked_vaa3d)
    delta = datetime.datetime.now() -start
