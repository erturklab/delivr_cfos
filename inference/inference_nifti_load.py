...
def create_empty_memmap (file_location, shape):
...
def run_inference(
    ...
    available_mem = int(available_mem*0.75) #safety margin
    ...
    #if you have nifti datasets (this will load the dataset into RAM)
    proxy_dataset = nib.load(niftis[0])
    #swap Y+Z for numpy-style ZYX representation 
    dataset = np.swapaxes(proxy_dataset.dataobj,0,1)
    ...
    output_image = create_empty_memmap (file_location = os.path.join(output_folder, comment,"inference_output.npy"), shape = dataset.shape)
    count_map = create_empty_memmap (file_location = os.path.join(output_folder, comment ,"count_map.npy"), shape = dataset.shape)
